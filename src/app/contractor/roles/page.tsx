              ...
              <tbody className="divide-y divide-slate-100">
                {roles.map((role: Role) => (
                  <tr key={role.reference} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      {role.is_head ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-jungle-100 text-jungle-700 uppercase tracking-wide">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-6 space-y-6 overflow-y-auto">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Role Title</label>
                  <input
                    {...register("name")}
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-start gap-4">
                  ...
                  <div>
                    <label className="font-bold text-slate-900 text-sm">Department Head Level</label>
                ...
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-700 ml-1">Assigned Permissions</label>
                            ...
                            return (
                              <div 
                                className={`p-3 rounded-xl border transition-all cursor-pointer flex items-start gap-3 ${
                                  isChecked ? 'bg-jungle-50 border-jungle-200' : 'bg-white border-slate-200 hover:border-jungle-200'
                                }`}
                                onClick={() => {
                              ...
                              >
                                ...
                                <div>
                                  <p className={`text-xs font-bold ${isChecked ? 'text-jungle-900' : 'text-slate-700'}`}>
                                    {perm.name}
                                  </p>
              ...
              <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50">
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-5 py-2.5 border border-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-jungle-600 text-white rounded-xl font-bold text-sm hover:bg-jungle-700 transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
                >
      ...
      {deletingRole && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center space-y-6">
            ...
            <div>
              <h2 className="font-black text-xl text-slate-900">Delete Role?</h2>
              <p className="text-slate-500 text-sm mt-2 leading-relaxed">
                Are you sure you want to delete the <span className="font-bold text-slate-700">{deletingRole.name}</span> role? Users assigned this role may lose access.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 px-4 py-3 border border-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 px-4 py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 shadow-sm"
              >
