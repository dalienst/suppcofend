...
type ContractorProductFormValues = z.infer<typeof productSchema>
...
export function ContractorProductForm() {
  ...
  const onSubmit = async (data: ContractorProductFormValues) => {
    ...
    try {
      ...
      router.push("/contractor/products")
    } catch (error: any) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {sitees?.map((b: any) => <option key={b.identity} value={b.identity}>{b.name}</option>)}
            </select>
          </div>
        </div>
      </div>
