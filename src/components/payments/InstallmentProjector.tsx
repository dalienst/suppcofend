        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-bold text-jungle-700">{months} Months</span>
              </div>
              <input
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-bold text-jungle-700">{depositPercent}%</span>
              </div>
              <input
          ...
          <div className="bg-slate-50 rounded-2xl p-6 space-y-4">
            <div className="flex justify-between items-center pb-4 border-b border-slate-200">
              ...
              <span className="font-semibold text-slate-900">KES {projection.depositAmount.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center pb-4 border-b border-slate-200">
              <span className="text-sm text-slate-500">Monthly EMI</span>
              <span className="font-semibold text-jungle-700">KES {projection.emi.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-500">Total Interest ({annualInterestRate}%)</span>
              <span className="font-semibold text-slate-900">KES {projection.totalInterest.toLocaleString()}</span>
            </div>
          </div>
        </div>

        <div className="space-y-4">
          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Payment Schedule</h4>
            ...
            {projection.installments.map((inst) => (
              <div key={inst.month} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl text-sm">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-[10px] font-semibold text-slate-500">
                    {inst.month}
                  </span>
                ...
                <span className="font-semibold text-slate-900">KES {inst.amount.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
